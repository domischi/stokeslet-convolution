...
@numba.jit
def rect_forcefield_to_center_non_normalized(x, a=1,b=1):
    x=np.array(x)
    if -a<=x[0] and x[0]<=a and -b<=x[1] and x[1]<=b:
        if norm(x)>1e-10:
            return -x
...
@numba.jit
def rect_forcefield_to_center_w_noise(x, a=1,b=1, sigma=.01):
    x=np.array(x)
    if -a<=x[0] and x[0]<=a and -b<=x[1] and x[1]<=b:
        if norm(x)>1e-10:
            return -x/norm(x)+sigma*np.random.normal(size=2)
    return np.zeros(2)
